...
- **Training the chat** (17 Sep 2026, Naomi: "I need all the possible responses to train the AI, and train it when I let you know"). `shopify-app/train-chat.py` builds every answer (179 on 17 Sep) from the live site: each residence page (status, suburb, rooms, sizes, features, opening lines), the Open homes & availability page (open homes, auction, selling agent; times that have passed are left out) and the Collection pages, plus the hand-written answers in `shopify-app/chat-training.json`. Without flags it only writes `docs/CHAT-TRAINING.md` for review. On her word, `--write` replaces the **Questions and answers** section at the END of /pages/inspections (everything above it is hers and untouched); it refuses if that section was edited in Shopify since the last run (`applied/chat-training-live.html`). Re-run it whenever open home times, statuses or answers change.
- **Never a street address** (STANDING, Naomi 17 Sep 2026: "never show the address, ensure it's private"; "if the IG says it it's fine but the AI should never state it nor should it say on website"). Suburb only for every house, for sale and in The Collection. `address_privacy.py` / `address-privacy.mjs` (one rule, street list in `address-privacy.json`) clean every caption the Instagram sync, `apply-series.mjs` and `add-series-blocks.py` write; `redact-addresses.py <theme> --write` cleans store data and theme templates and runs last in push-live; `find-addresses.py` lists anything left. A new house on a new street goes into `address-privacy.json` so its suburb survives the clean.
